...
from keras.api.layers import Dense, Dropout, Activation, AveragePooling2D, MaxPooling2D
from keras.api.layers import Conv1D, Conv2D, SeparableConv2D, DepthwiseConv2D
from keras.api.layers import BatchNormalization, LayerNormalization, Flatten
from keras.api.layers import Add, Concatenate, Lambda, Input, Permute
from keras.api.regularizers import L2
from keras.api.constraints import max_norm
import keras
from keras import backend as K

from Model_Functions.attention_models import attention_block
...
def ATCNet(n_classes, in_chans=22, in_samples=1125, n_windows=5, attention='mha',
    ...
    if (fuse == 'average'):
        if len(sw_concat) > 1:  # more than one window
            sw_concat = keras.layers.Average()(sw_concat[:])
